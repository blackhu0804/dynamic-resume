...
/* 这种格式太难看了，我们让他转换成markdown格式吧O(∩_∩)O哈哈~ */
`

const showStyles2 = (callback) => {
  let n = result.length;
  let id = setInterval(() => {
    if (n >= result2.length) {
      ...
      text.innerHTML = result2.substring(0, n);
      text.innerHTML = Prism.highlight(text.innerHTML, Prism.languages.css);

      cssCode.innerHTML = result2.substring(0, n);
      text.scrollTop = 1000;
      n += 1;
    }
  }, 100)
...
const markdownToHtml = (callback) => {
  let resumeTextMD = marked(resumeText);
  resume.innerHTML = resumeTextMD;
}
